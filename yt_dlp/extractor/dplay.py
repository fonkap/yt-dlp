...
class DMaxIE(DiscoveryPlusBaseIE):
    _VALID_URL = r'https?://(?P<domain>dmax\.marca\.com)/series' + DPlayBaseIE._PATH_REGEX
    _DISCO_CLIENT_VER = '2.1.9'
    _PRODUCT = 'wbdatv'
    _DISCO_API_PARAMS = {
    ...
    _TESTS = [{
        # non geo restricted, via secure api, unsigned download hls URL
        'url': 'https://dmax.marca.com/series/curiosidades-de-la-tierra-espana/season-1-episode-3',
        'info_dict': {
            'id': '8001',
            'display_id': 'curiosidades-de-la-tierra-espana/season-1-episode-3',
            'ext': 'mp4',
            'title': 'Episodio 3',
            'description': 'md5:1bb10a759bcb46bf9b499ceda22af996',
            'duration': 2640.16,
            'timestamp': 1728335640,
            'upload_date': '20241007',
            'creator': 'dmax',
            'series': 'Curiosidades De La Tierra: España',
            'season_number': 1,
            'episode_number': 3,
            'thumbnail': 'https://images.aurora.enhanced.live/es/images/video/EHD_488836B/default.jpg',
            'tags': [],
            'creators' : ['dmax'],
            'season': 'Season 1',
            'episode': 'Episode 3',
    ...
    def _get_disco_api_info(self, url, display_id, disco_host, realm, country, domain=''):
        ...
        show_sn = display_id.split('/')[0]
        episode_sn = display_id.split('/')[1]

        try:
            show = self._download_json(
                disco_base + 'site/shows/' + show_sn, display_id,
                headers=headers, query={
                    'include': 'default',
...
class DMaxShowIE(DPlayBaseIE):
    ...
    _TESTS = [{
        ...
        'playlist_mincount': 8,
        'info_dict': {
            'id': 'curiosidades-de-la-tierra-espana',
        },
    }]
    ...
    def _entries(self, url, show_name):
        headers = {
            'Referer': url,
            'Content-Type': 'application/json',
        }
        ...
        show_json = self._download_json(
            f'{self._BASE_API}site/shows/{show_name}/',
            video_id=show_name, headers=headers, query={
                    'include': 'default',
                    'filter[environment]': 'dmaxspain',
                    'v': '2',
                    'parent_slug':'series',
        ...
        episodes_json = show_json['blocks'][1]['items']
        for episode in episodes_json:
            video_path = episode['meta']['embedUrl']
            yield self.url_result(
                video_path.replace('{settings.site.info.domainLink}', self._DOMAIN),
                ie=self._VIDEO_IE.ie_key(), video_id=episode.get('id') or video_path, title=episode.get('title'))
